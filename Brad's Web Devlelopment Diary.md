...
- 백신휴가 2일차
  - 모니터 수령 및 확인
    ...
    - 코드 확인 편리로 업무 효율성 향상 기대
    
  - 웹디자인기능사 학습
    - 준비 서적 도착
    - 한 달 뒤 자격증 시험 조기 대비 실시
...
- 모니터암 설치
  - 한성 32인치 QHD모니터에 설치
  - 베사홀의 위치가 기본 제공 모니터 받침대 제거시 안쪽으로 들어간 형태로 지지 나사 활용하여 설치
  - 높이가 맞지 않아 외관상 안정감이 있지는 않지만 사용에는 문제가 없음
  - 기본 받침대의 높이가 낮은 점을 보완하고 업무시 적절한 높이로 수시로 변경이 가능한 점이 최대 장점
  - 월요일부터 업무 능력 향상을 기대
  
- 뷰 복습
  - 한번 학습했던 뷰를 복습
  - 이후 개인 프로젝트 진행에 활용여부를 판가름 중
  
- 웹디자인기능사 필시 준비
  - 생각보다 디자인 관련 부분의 이론이 많아 어제 고민을 하게 됨
  - 뷰 복습 이후에 빠르게 이론 정독하고 하루에 기출 한개씩 푸는 방법으로 시험전까지 준비 전략을 세움
  - 60분에 60문제를 푸는 시험으로 난이도는 높지 않을 것으로 예상이 되어 하루 1시간 이내로 꾸준히 준비하면 합격이 가능할 것으로 예상
...
- 모니터 설치 준비
  - 모니터 분리, 모니터암 가방에 준비
  - 내일은 평소보다 이른 출근 후 세팅 완료 예정
  - 화면단에서 엑셀 작업이 많아질 예정으로 효율성 증가를 목표로 함
  
- 반영 사항에 대한 추가 작업 예정
  - DB확인 및 대응 수정 작업
  - 구현된 소스들의 점검 
  - 상세페이지 외 가능한 작업을 모두 마무리 짓는 작업
  - 추석전에 상세페이지를 제외한 모든 페이지의 완벽한 완료를 목표
...
## 2021-09-13 업무일지
- 모니터 설치 완료
  - 32인치로 대부분의 소스를 가로로 잘림 없이 확인 가능
  - 소스 비교 및 참고가 훨씬 수월하여 눈이 덜 피로함
  
- 디비 변경 사항 확인 및 대응
  - 컬럼의 수정, 이동으로 인한 쿼리 오작동 확인 및 수정 업데이트
  - 컬럼 추가 필요한 사항 반영 및 not null 제약으로 인한 데이터 임포트 오류 대응
  
- 파일 업로드 기능 개발 시작
  - 천과장님 소스와 산업용수 소스 비교
  - 천과장님 소스로 먼저 작업 시작하여 파일 조회부터 구현 시작
  - 다른 개발팀의 소스도 참고하면 더욱 많은 api 확인 가능
  - 구현 방법에 있어 여러 가지 방향이 가능함을 인지
  - 보유기술쪽의 한 건의 업로드로 구현 테스트 진행 후 프로세스 파악이 되면 기술성과 쪽의 다량의 파일업로드를 일괄로 진행 예정
  
---

## 2021-09-14 업무일지
- 파일 업로드 구현중
  - 화면단의 처리와 백앤드 단의 처리 과정을 이해
  - 소스 구현 중 오류들을 모두 해결하지 못한 상태로 퇴근
  - 내일 출근 후 바로 테스트 예정
  
- 화면단 CUD 기능 구현 필요
  - 보유기술 쪽 상세 화면에서 먼저 구현 예정
  - 연구과제 파트에서 연계된 화면이 존재
  - 협업을 위한 업무 순서
  
- 협의 없이 늘어난 분량
  - 일의 진행과정을 통해 확실히 본받지 말아야할 유형의 부장 타입을 각인시킴
  - 앞으로 성장에 있어 기억하고 피해야할 사례
...
- 파일 업로드 구현
  - 다건의 경우 추후 구현 예정
  - 섬세한 기능 적용을 위해 화면 분석을 다시 시작
  
- 화면단의 수정 사항 반영
  - 현업 수정 의견 반영한 소스 업데이트

- 디비 변경으로 인한 쿼리 재작성
  - 디비 초기화로 인한 조회 오류가 다시 발견된 부분에 대한 수정 조치

- 개발자간 의사소통
  - 상호간의 소통을 통해 구현 완료한 기능들에 대한 공유
  - 개발 작업의 효율성 증가
  - 혼자 해매는 시간의 단축
...
## 2021-09-16 업무일지 
- OZ 보고서 
  - DB 연결 오류로 연결까지는 시도하지 못함
  - 화면 단에서 양식 입력까지는 완료
  - 업무 프로세스는 파악하여 DB 연동시 테스트 가능 예상
  
- 상세 페이지
  - 업무 프로세스가 확실히 잡혀야 CUD, 권한별 작업이 가능하여 분석중
  - 화면단에서 필요한 스크립트 참고 및 분석
  - 동일 기능 구현에 사용된 다양한 방법의 참고로 사고의 범위를 넓히는 것이 주요함
  
- 의사소통 효율성
  - give and take 관점에서 give가 많아지는 경우
  - 시간적 손해를 보지 않는 선에서 절충의 필요성

- 개발 능력
  - 모든 의사소통의 힘은 자신의 능력으로부터 기인
  - 부족한 부분을 바로 직시하고 채워가는 것이 중요
...
## 2021-09-17 업무일지 
- OZ 보고서 DB 연결
  - 설정 경로 파일 수정시 일반 편집기로 수정, 저장한 사항은 반영이 안되었던게 문제
  - 관리자 권한으로 수정을 해야 수정된 설정 사항이 반영됨
  
- TAB별 저장 프로세스
  - 텝 아이디를 파라미터로 넘겨서 서비스 임플레먼트에서 분기 처리
  - 관리번호 등 PK값 신규 입력 방법 확인
  - 상세 CUD 작업을 명절 후 반복하여 권한단을 제외한 상세 페이지 개발 작업 완료 목표
  
- OZ 보고서
  - 매핑하는 방법은 이해함
  - 단건을 단순하지만 다건의 연속 출력일 경우 테스트가 필요

- 결재, 기안
  - 확실하게 프로세스가 정리된 페이지에 테스트로 1회 구현 목표


## 2021-09-18 연휴 중 학습
- 웹디자인 기능사 필기시험 준비
  - 10월 8일 오전 9시 50분 시험
  - 3주 정도 남은 시간으로 여유롭지는 않은 일정이지만 틈틈히 학습 필요
  
- 구매한 도서 확인
  - 현재까지 구매한 도서들에 대한 학습 계획을 웹디자인 기능사 시험 이후 수립 필요
  - 우선순위는 SI 현업에 필요한 사항과 관련된 분야를 최고로 놓고 다른 분야도 계획 수립 예정
  - 리엑트, 리엑트 네이티브, 뷰 등의 프론트앤드 프레임워크도 높은 흥미가 있음
  - 컴퓨터 공학과 기본 소양 과목들도 이수를 희망
  

## 2021-09-20 연휴 중 학습
- 웹디자인 기능사 필기시험 준비
  - 이론 파트 학습 완료
  - 디자인 부분은 full 암기보다 기출로 유형만 체크하고 웹 관련 부분은 복습
  - 생각보다 디자인 부분을 제외한 나머지 부분은 학습에 큰 어려움이 없을 것으로 예상됨
  
- 시험 준비를 제외한 시간의 계획
  - 뷰 복습 마무리
  - 리엑트, 리엑트 네이티브 학습 예정
  - 타입스크립트 학습 이어가기
  - 위 사항을 우선적으로 수행하면서 블로그 제작 시작
...
- 연봉올리기 프로젝트
  - 개발 능력 향상
    - 자격증 구비 : SQLD, SQLP, 웹디자인 기능사 등의 자격
    
  - 개발언어
    - JAVA, JavaScript, TypeScript, css, html, Python, mySQL, Oracle
    
  - 프레임워크
    - React, Vue, React Native, Spring, Node.js, 전자정부프레임워크, 웹스퀘어
    
  - 운영체제
    - 리눅스, 맥, 윈도우
    
  - 컴퓨터공학
    - 자료구조와 알고리즘, 컴퓨터 구조 및 설계, 운영체제
...
- 알고리즘 데이터 구조 학습
  - 노마드 코더 유튜브 활용
  - 버블정렬, 선택정렬, 삽입정렬, Big O, Array CRUD 성능, 시간복잡도 등의 개념을 학습
  - 장기적으로 데이터, 자료구조와 알고리즘을 학습할 필요성을 느낌
...
- 보고서 출력 기능
  - 쿼리 작성 프로그램의 오류로 오전에 작업 시도하다 연기함
  - 상세페이지의 화면과 보고서 양식의 데이터가 상이하여 쿼리를 다시 짜야하는 상황 인지
  - 다음주에 작업 예정
  
- 상세페이지 CUD
  - type별 권한 처리 프로세스 적용
  - row status 활용한 프로세스 적용
  - 상세페이지 들에서 필요한 CUD 모두 구현 예정(파일업로드 포함)
...
## 2021-09-26 업무일지
- 주말 육아
  - 연후를 재울 때 운동겸 계속 걸어서 토요일은 뻗음
  - 걷기 운동을 많이 해서 평소보다 체력적으로 힘든 주말
  - 개인 시간은 거의 없었음
  - 웹 디자인 기능사 기출 1회 풀이 결과 60문항 중 5개 오답으로 36개 정답이면 합격이기에 준비에 여유가 생김
  
- 웹 디자인 기능사
  - 여유로운 준비가 가능한 난이도의 시험
  - 매일 기출 1~2회를 소화하고 시험에 임하면 충분할 듯
  
- SQLP 시험
  - SQLD 시험 발표(10월 1일) 결과가 합격인 경우 바로 접수
  - 11월 20일 SQLP 시험 준비를 웹 디자인 기능사와 함께 준비 고려중
  - 실무에서 쿼리 작성 능력이 반드시 필요함
  - 설계 부분에서도 고려할 사항들이 시험 내용에 포함이 되어 있어 실무와 학습이 유기적으로 연결이 됨
  - 이후 맡을 수 있는 업무 스펙트럼을 키우기 위해서도 취득하면 유리한 자격증으로 보임
...
## 2021-09-27 업무일지 
- 상세페이지 CUD
  - 멀티파일 업로드 기능을 텝별로 구현중
  - 화면단 스크립트 및 자바단 서비스 임플레먼트 로직이 복잡함
  - 현재 비슷하게 구현했지만 정상 작동은 되지 않고 있는 상황
  - 내일 소스 코드 다시 분석하고 재도전 예정
  - 이번 주 내로 상세페이지 CUD와 보고서는 모두 완료 목표
  
- 화면단 UI 문의
  - 내일 오전 중에 문의 예정
  - STPE 개발도 순차적으로 진행 필요
  - 현재 분석 및 설계 수준 파악이 중요
  
- 자바 자체에 대한 이해 증진 필요
  - 그동안 스크립트에 치중했지만 권한과 상태 값에 따른 로직 구현시 백앤드 단에서 자바 코드가 필요함
  - 익숙하지 않음과 복잡함으로 인해 이해가 더디고 적용은 더 늦은 상태
  - 하루 하루 참고 견디면 익숙해지고 능력이 되기에 꾸준히 도전하는 자세가 중요
...
- 파일업로드 구현
  - 생각보다 오류와 실수로 진도가 더딤
  - 결국 프로세스와 단계별 구현이 필요함
  - 주중에 완료가 목표
  
- 팝업 화면의 선택 정보 매핑
  - 상세 화면의 각 팝업창에서 선택한 사항의 반영 구현 필요

- 세부적인 기능들의 구현
  - 상세 페이지들의 완성도를 높이기 위해 디테일을 잡아가는 것이 필요한 시기가 됨
  
---

## 2021-09-29 업무일지 
- 상세페이지 팝업 선택 사항 매핑
  - 상세페이지의 팝업을 통한 입력 사항을 부모 페이지에 매핑하는 작업 수행
  - 파라미터 값을 받아 활용하는데 아직 미흡한 점이 있음
  - 화면단에서 처리되는 정보들을 보면서 업무 로직 파악을 해봄
  
- CSS 링크 정리
  - 초기에 정리 되었어야하는 부분을 이제야 발견하고 수정함
  - 전체적인 공통소스 적용은 소스 파일이 아닌 설정 파일에 적용함을 인지함
  
- 보고서 출력 기능
  - OZ 프로그램 활용 능력이 필요함
  - 코딩 능력보다는 새로운 프로그램 활용 능력이 필요한 부분으로 보고서 완성 직전 수준까지 작업함
  - 내일 오전 중으로 단건 보고서 출력 기능은 완료 가능할 것으로 예상
  
- 완료 수준
  - 기안, 결재 등의 기능을 제외한 나머지 기능 구현 완료시 공정 완료 표기
  - 권한, 상태값에 따른 분기 처리는 가능한 상황이 된 시점에 처리
...
- 오즈 보고서 출력 기능 구현
  - 출력까지 테스트는 완료
  - 화면단에 필요한 정보를 가져오기 위한 쿼리 작성
  - 테이블이 변동될 예정이기에 확인 가능한 선에서 파악 및 쿼리 작성
  - 컬럼 확인에 많은 시간이 소요

- 다건 출력 기능 구현
  - 내일 오전 중으로 구현 시도
  
- 팝업창의 정보를 모두 상세창에 매핑하는 작업
  - 상세 CUD 완료를 하기 위한 사전 작업으로 필요
  - 스크립트의 좀 더 세부적인 사항까지 이해 요망
...
## 2021-10-01 업무일지
- 상세페이지 팝업 매핑 완료
  - 과제 선택과 관련한 부분을 제외한 모든 팝업 매핑 구현
  - CUD에서 필요한 정보를 쿼리로 전달할 필요하 있는 경우 수정이 필요
  - 화면단에서 보이는 정보와 저장시 필요한 정보가 상이한 경우에 대한 보완 필요
  
- 오즈 보고서 쿼리 완료
  - 키워드 매핑 쿼리 구현
  - 오즈 프로그램의 성능이 낮아 시간이 많이 소요되어 다음으로 연기
  - 구현 방법은 숙지 완료
  - 다건 출력 시도 예정
...
## 2021-10-03 업무일지
- 연휴 2일차
  - 1일차는 연후를 재우면서 같이 뻗어버림
  - 웹 디자인 기능사 기출 문제 13회까지 모두 품
  - 13회 전체 평균 83점으로 60점 이상의 합격선은 무난히 넘을 것으로 예상
  - 남은 기간 이론 1일, 실전모의고사 총 2회, 오답 정리를 일 2시간 이내로 소화하고 시험 예정
  - 금요일 오전 9시 10분 시험으로 반차 예정
  
- SQLP 시험
  - 현재 가장 빠른 시험일정은 11월 20일로 접수는 10월 25일
  - 남은 기간 동안 소화할 수 있는 학습량인지에 대한 판단은 24일까지의 학습량을 기준점으로 설정
  - SQLD 대비 난이도나 분량이 많기에 무리한 도전보다는 합리적인 도전이 더욱 의미 있을 것으로 예상
  
- 개인프로젝트
  - 10월부터 개인 프로젝트가 가능한 경우 시험의 일정도 고려해야함
  - Vue.3 복습 이후 리엑트 학습 예정
  - 시간이 허락하는 경우에 한해 운영체제 등의 컴퓨터 공학 기본 과목들에 대한 학습도 필요
...
## 2021-10-04 업무일지
  - 주간 계획
    ...
    - 상세페이지 기능 구현 예정
    - 권한, 상태값에 따른 로직 구현
    - 시간적 여유가 있는 경우 보고서 구현 다시
    
  - 다음주 계획 
    - 월요일 대체공유일, 목요일과 금요일은 백신 휴가로 실제 업무일은 화, 수요일 2일간
    - 쿼리 업데이트와 코드 리펙토링 예정
...
## 2021-10-05 업무일지
  ...
  - 상세페이지 보정
    - 매핑되는 내용 및 스크립트 실행 오류 보정
    
  - 다건 보고서 출력
    - 선택 리스트 파라미터들을 모아서 준비까지 진행
    - OZ 리포트에서 화면상에 다건 화면 출력의 경우 지원하는지를 먼저 확인해야함
    - 현재까지는 화면상이 아닌 지면 출력시에 멀티건은 가능한 것으로 파악
    
  - 내일 일정
    - 상태별 소스 적용 분석, 파악(개발 완료팀 조언을 구할 예정)
    - 오아시스 결재 파트 분석, 파악(개발 완료팀 조언을 구할 예정)
    - 상세화면 CUD를 위해 필요한 테이블별 컬럼 파악 및 쿼리 수정(컬럼추가, VO, 데이터리스트 수정 필요)
...
## 2021-10-06 업무일지
  ...
  - 오즈 보고서 분석
    - 다건 출력의 경우 공통팀에 문의 후 진행이 필요
    
  - 오아시스 결재
    - 오아시스 결재 방법에 대해 산업용수 노충호 대리님으로부터 교육받음
    - 흐름 자체는 이해를 하였으나 구현에 있어 시간은 걸릴 것으로 예상
    
  - 의사소통
    - GIVE and TAKE
    - 듣고자 하면 들어야 한다
    - 시간 관리의 효율성을 위한 대처법도 배움의 연장선에 있음
  
  - 미래기술 팝업 내용 매핑
    - 보유기술 상세화면을 위한 매핑
    - CUD 프로세스를 위한 마지막 작업
    - 권한, 상태값에 따른 분기 처리까지 하면 CUD 작업 가능
    - 18~22일 사이에 상세 페이지 CUD 완료 예정 
...
## 2021-10-08 업무일지
  ...
  - 오전 반차
    ...
    - 20분 정도 소요
    - 85점 51/60 으로 합격
    - 실기는 수자원 프로젝트 이후 응시 예정
    
  - SQLP 준비
    - 본격적으로 준비 시작
    - 수자원 프로젝트에서도 분석,설계 단에서 도움이 되는 내용으로 파악
     
---
## 2021-10-09 업무일지
  - SQLP 학습
    - 국가공인 SQLP 자격검정 핵심노트 1, 2
    - 문제풀이를 통해 오답노트를 하며 이론을 학습하는 방법으로 공부
    - 필요하면 추가 교재 구입을 통해 SQL 관련 역량을 키우기
  
  - 분석, 설계 분야
    - 분석, 설계 파트에 대한 학습을 통해 단기간에 직무 변환을 목표
    - 웹 개발에 있어 전반적인 과정을 모두 이해하고 능숙하게 개발하고자 하는 개인적인 목표 수립
    
  - AI 분야
    - 현재는 웹 개발에 관한 능력을 키우기 위한 공부에 집중
    - 관심을 갖고 꾸준히 시간을 투자가 필요
    
---
## 2021-10-10 업무일지
  - SQLP 학습 
    - 육아로 많이 하지 못함
    - 꾸준히 쉬지 않는 것이 중요하다고 생각...
...
## 2021-10-12 업무일지
  - 다건 보고서 출력
    - 화면단 스크립트와 오즈 쿼리 수정으로 구현 가능
    - 거버넌스 파트에서 구현한 소스 참조
    
  - 상세화면 CUD 구현을 최우선 순위로 수정
    - 11월 오픈을 염두한 개발 계획 수정으로 CUD를 우선적으로 구현 후 세부 사항 적용 예정
    - 보유기술 CUD가 보다 간편하여 우선적으로 시작
    - 기술실용화 파트는 탭별로 저장 프로세스가 필요하여 공수가 추가로 필요
    
  - 결재, 기안
    - 상세 CUD를 우선으로 작업하고 기능을 추가하는 방향으로 설정
    - 공수일정 업데이트가 한달간 없는 점을 염두한 일정 수정

---
## 2021-10-13 업무일지
  - 상세화면 CUD
    - 컬럼 매핑에 추가 시간 소요
    - VO, 데이터리스트, 데이터맵, 쿼리, 웹스퀘어 바인딩의 연속 작업으로 인한 공수 추가
    - 업데이트도 완료하지 못함
    - 다음주부터는 휴가가 없는 관계로 CUD구현에 올인
    - 완료 페이지 수 10페이지 이상 목표
  
  - 현업 수정 요청 사항 반영
    - 현재까지 미반영된 부분 반영 예정
    - 추가 스크립트 등 세밀한 부분까지 구현 필요


---
## 2021-10-17 업무일지
  - 18~22일 개발 계획
    - 보유기술, 기술실용화 상세페이지 CUD 개발 완성
      - 팝업 매핑 완료
      - 컬럼 매핑 완료
      - 파악된 권한, 상태값에 따른 기능 구현
      - 파일 업로드 구현
      - 텝별 프로세스 구현
...
## 2021-10-18 업무일지
  - 테이블 컬럼 수정사항 반영
    - 웹스퀘어, 쿼리, VO, 스크립트 모두 수정
    - 완성된 페이지에 다시 매핑 작업으로 공수 추가
   
  - 보유기술 상세 업데이트
    - 보유기술 메인 테이블 업데이트 구현 완료
    - 보유기술 상세의 CUD부터 완벽히 구현 후 기술성과관리 상세 개발 예정
    - 단일 화면의 보유기술 상세를 마치면 기술성과관리 상세는 보다 빠른 진행이 예상됨
...
## 2021-10-19 업무일지
  ...
  - 채번 쿼리
    - 보유기술관리 번호 채번 쿼리
    - 다른 번호와는 달리 채번 쿼리가 바로 드러나지 않음
    - 천과장님의 도움으로 쿼리 수사 작전 성공
    - AS-IS 실제 저장을 통한 조건별 쿼리 추적
    - 2단계에 걸친 채번 과정에 사용된 소스 추출 및 TO-BE 적용
    - 고난도 소스 분석 과정을 옆에서 경험함
    - 내일 처음부터 분석을 다시 해보고 그 과정을 내제화하는 것이 필요
    
  - 2주간 계획
    - 천과장님이 나가기 전에 난이도 높은 파트에 대한 개발을 마치는 것을 목표
    - 가능한 날은 야근을 통해서라도 구현이 필요
  
 
---
## 2021-10-20 업무일지
  - 채번 쿼리
    - 보유기술관리번호 채번 쿼리 소스 파악 과정 복기
    - 백단의 2번의 쿼리 적용 및 화면단의 1번 적용되는 채번 관련 스크립트 작성
    - 저장 프로세스에 필요한 작업 완료
    
  - 상태값 코드
    - 화면 상태값에 따른 저장, 업데이트, 삭제 구분 실행
    - 이벤트에 따라 상태값 변경 코드 삽입 필요
    - 업무 프로세스에 따른 적용 시작
...
## 2021-10-21 업무일지
  - 보유기술 상세화면
    - 화면단 과제선택 매핑
    - 키워드를 쿼리에 적용가능한 형태로 변환하는 스크립트
    - 유효성 테스트 추가
    - insert 쿼리 작성
      - 널포인트 에러 발생으로 내일 수정 요망
        
---
## 2021-10-22 업무일지    
  - 보유기술 상세화면
    - 저장, 업데이트를 상세 페이지의 모든 테이블에 적용
    - 권한별 적용이 필요한 다음 페이지의 경우 권한 및 상태 파악이 필요
    
  - 야근
    - 리스트 형태의 리퀘스트 데이터를 담아 활용하는 방법에 대한 테스트
    - 리스트 형태의 다건 데이터를 저장하는 형태 테스트
    - 상세페이지 중 구현 난이도가 높은 페이지 분석 : 도움이 절대적으로 필요
...
## 2021-10-25 업무일지    
  - 보유기술 상세화면
    - 등록화면은 완료 처리
    - 이후 화면단은 권한 및 상태값이 필요한 상황으로 현업 확인 내용이 필요함
    - 컬럼명 수정으로 인한 수정 작업
    
  - 보유기술 등록이력 팝업
    - 팝업창에 부모창에서 클릭한 보유기술에 대한 등록 이력을 조회하는 화면 개발 완료
    
  - 실용화현황
    - 실용화 현황 목로 페이지에서 클릭한 실용화기술에 대한 팝업
    - 실용화 기술의 맵을 그려주는 동적 페이지 구성 필요
    - 맵의 기반이 되는 데이터 분석 후 화면을 그리기 위한 소스 분석이 필요
    - 구현까지는 최소 2일 이상 소요될 것으로 보임
...
## 2021-10-26 업무일지 
  ...
  - 실용화현황 상세 페이지
    - 동적 화면 구성 소스 분석
    - 데이터 구조를 파악해야 동적으로 구성할 수 있는 로직 완성 가능
    
  - 실용화 데이터베이스
    - 실용화현황의 동적 화면 구성을 위한 AS-IS 소스 분석중 누락 테이블 발견
    - 채번에 활용되는 쿼리로 AS-IS 상세화면에 사용되던 테이블
    - 내일 관련 쿼리, 화면 등 확인 후 확실한 내용 파악 예정
    
    
---
## 2021-10-27 업무일지 
  - 실용화현황 상세페이지
    - 쿼리의 반정도 작성하는데 반나절 소요
    - 상세페이지 분석을 통한 실용화 업무 전반에 관한 프로세스 이해도를 높임
    - 업무 분석이 중요한 이유를 다시 한번 깨달음
    
  - 메일로 소통
    - 설계자와 소통을 메일을 통해 객관적인 근거가 남을 수 있도록 함
    - 명확히 요청 시점과 처리 시점을 기록할 수 있음
    - 질문의 기록으로 제 3자의 검증을 받기도 훨씬 수월함
...
## 2021-10-28 업무일지 
  - 실용화현황 상세페이지
    ...
    - 쿼리 결과 화면 매핑 완료(하단부)
    - 동적 화면 그리기 도전 과제
    - 쿼리 결과에 따른 그리드 동적 구현이 필요
    
  - 결재 기능
    - 진부장님의 도움으로 결재 파트에 대한 업무 이해도를 높이는 중
    - 보유기술 화면에서의 승인요청 프로세스에 적용 예정
    - 결재 기능을 이해하면 실용화 파트의 탭별 결재 프로세스 구현 가능
    
  - 권한 및 상태값
    - 상세 페이지 세부 구현에 필요한 권한과 상태값이 필요
    - 설계자에게 요청 후 확인이 필요
    - 공통코드 등 필요한 코드 등에 대한 요청 
...
## 2021-10-29 업무일지 
  ...
  - 실용화추진요청
    - 요청페이지 목록 조회 프로세스 협의
    - 사라진 2개의 요청 관련 테이블을 대체하는 프로세스 확인
    - 현업에 대한 확인이 필요한 사항
    
  - 천과장님 계약 마지막 날
    - 멘토였던 천과장님의 마지막 날로 배웅
    - 배울 점이 많았던 개발자로 개발 뿐만 아닌 소통에 관한 부분도 인상적임
    - 다시 함께 일하는 날이 오기를 기대
...
## 2021-10-31 업무준비
  - 재계약 대비 데스크탑 서치
    - 듀얼모니터 구성
    - 그래픽 카드 가격이 너무 높아서 내장그래픽으로 사용
    - 100만원대로 구성중
    - 이후 그래픽카드만 추가 혹은 CPU만 교체하면 무거운 작업 가능하도록 기본 세팅
    - 저장, 처리 속도를 최우선으로 구성
    - 재계약 시점의 시세로 바로 구입 예정
    
  - 11월 첫 주 업무 계획
    - 실용화현황 상세페이지
      - 동적 페이지 렌더링 구현
      - 클릭 이벤트 구현(웹스퀘어라 구현이 힘듬)
      - 위 2가지 기능 완료시 1개 상세페이지 완료
      - 2일 이내 구현 목표
    ...
    - 실용화추진요청 상세페이지
      - 실용화 메인 상세페이지로 텝별 저장부터 구현 시작
      - 텝별 저장 구현 후 업데이이트, 삭제 구현 (읽기는 모두 구현 완료)
      - 기능 구현 우선 적용
      - 권한별, 상태별 프로세스 적용은 해당 내용 확인 후 진행
      
    - 보유기술, 실용화 상세페이지 완료 계획
      - 11월 중으로 상세페이지 권한, 상태값 적용 완료 목표
      
    - 미비된 페이지 완료
      - 11월 중으로 확인 필요한 사항 체크 완료 및 개발에 사용되는 DB 확인 목표
      - 2개의 목록 페이지
        - 이미지를 다수 슬라이드, 썸네일로 제공하는 페이지
        - 지도 등 도표로 현황을 제시하는 페이지
          - AS-IS 데이터를 확인할 수 없는 상황 
...
## 2021-11-01 업무일지
  - 실용화현황 상세페이지
    - 하단 매핑 정보 조회 쿼리 수정
    - 공통코드에 누락된 코드들로 인한 표기값이 없는 문제 확인
    - 매핑된 정보를 활용한 동적 소스 구성 준비 완료
    - 내일(화요일) 구현 테스트 시작
    - 동적 구현 완료시 클릭 이벤트 스크립트 추가
    
  - 기안
    - 오아시스 기안 소스를 진석필 부장님께서 도와주심
    - 현재 90%이상 구현된 상태로 커스터마지징만 하면 바로 사용이 가능함
    - 실용화 상세페이지 텝별 업무 프로세스 적용시 활용 가능
...
## 2021-11-03 업무일지
  - 실용화현황 상세페이지
    - 상단부 동적 구현 완료
    - 동적 구현부에 스크립트까지 적용
    - 클릭 이벤트 팝업 및 하단 그리드 정보 재매핑 구현
    - 현재까지 구현한 페이지 중에서 가장 복잡하고 난이도 있는 작업
    - 자바 서비스 단에서 동적 변수 활용한 for문 활용하여 단일 쿼리로 다건 조회 구현
    - 쿼리의 중복을 최소로 줄여주는 로직 사용
    - 화면단에서 렌더링 함수를 최소로 하는 로직 사용
    - eval() 함수 첫 사용
    - 동적변수 활용으로 렌더링 함수를 한번만 정의해서 재사용함
    
  - 목록 페이지 이미지 요청
    - 메일로 이미지 요청 및 일정 관련 이슈 제기
    - 관리자에게 참조로 송부
...
## 2021-11-04 업무일지
  - 재계약
    - 450만원에 재계약 협상중으로 두 업체간 협의가 중요
    - 일관성있게 양측에 연장과 연봉인상 조건을 얘기했지만 해석의 차이로 인한 혼선
    - 원만한 결론을 기대
  
  - 점검 사항 반영
    - 만들어진 페이지에 대한 부분적 기능 점검 사항에 대한 피드백 및 해당 사항 수정
    
  - 실용화현황 상세페이지
    - 동적 구현 완료했으나 특정 조건에서 그리드 렌더링이 틀어지는 오류 발생
    - UI단 문제로 공통팀에서 수정 후 다시 수정 작업이 필요
...
## 2021-11-05 업무일지
  - 재계약
    - 대표님과 점심, 오후 미팅
      - 퇴사 후 프리로 재계약 하는 방향으로 확정
      - 주변 소식으로 인한 오해를 해소하고 올바른 상황 인식이 가능하도록 정보 제공
      
  - 보유기술 분류 목록 페이지
    - 핵심기술, 일반기술 개수 매핑
    - 썸네일 이미지 소스 반영
    - 팝업 이벤트 구현 예정
    - 팝업시 표기 형식에 대한 협의 필요(단건 혹은 다건 슬라이드)
    

---
## 2021-11-08 업무일지
  - 재계약
    - 금요일 퇴사 얘기가 정직원 퇴사이고, 연장은 ecl 프리로 재계약이었다는 소리에 황당함
    - 의사소통에 있어 문서화가 얼마나 중요한지 다시 한번 느낌
    
  - 전체회의
    - 기술개발 화면 분석을 수행하는 사람이 없는 부분 이슈화
    - 해당 파트에 대한 PI 직접 소통이 공식화 됨
    - 앞으로의 변화에 대응하기 위한 화면 설계에 대한 대비가 필요
    - 현업과 소통에 대비한 9월 7일 기준 협의 내용 반영 최우선 조치
    - 금주 내로 1차 소통이 목표
    
  - 상세페이지 개발 준비
    - 현재까지의 화면정의서, 회의내용을 정리하여 소통 내용을 문서화하여 정리 예정
    - 목록 1개 페이지 구현 필요 : 지도가 포함된 3개의 도표가 있는 화면
    - 상세페이지에 구현이 필요한 로직과 관련된 소스 분석 및 파악이 선제되야 함


---
## 2021-11-09 업무일지
  - 슬라이드 팝업
    - 공통팀 협조로 레이아웃 소스 받고 실제 이미지 파일로 구현
    - 슬라이드 동적 구현은 포기하고, 소스 날코딩으로 구현
    - 클릭한 이미지 정보값을 팝업창으로 넘기는 방법을 찾아 해맴
    - this 를 사용하여 id 값을 찾았으나 퇴근전 영문없이 숫자로만 아이디로 해결하려다 실패
    - 숫자만으로 아이디를 정한 경우 웹스퀘어에서 자동으로 랜덤 아이디 생성해버림
    - 팝업창에서 숫자로 인식 못하는 오류가 발생
    - 영문포함 뒷자리채번 방식으로 수정하여 내일 해결 시도 예정
    
  - 일정
    - 슬라이드 팝업 작성 완료 시 페이지 한개 완료
    - 워드 클라우드 팝업 또한 일정에 반영
    - 팝업 클리어시 일정 완료로 반영
    - 현업 회의사항 반영을 통해 다음주 현업 및 PI와 소통 준비
    - 상세 페이지를 염두한 질의 내용 정리가 필수
...
## 2021-11-10 업무일지
  - 슬라이드 팝업
    - 어제 구상한 방법으로 구현 성공
    - 이미지를 받아 교체하면 개발 완료이기에 완료 처리함
    
  - 실용화현황
    - 이미지 클릭시 팝업되는 상세페이지의 동적 그리드 부분 구현 완료
    
  - PI 소통 대응 준비 작업
    - 9월 7일 회의 내용 반영 여부 확인
    - 화면정의서 기반 업무 프로세스 재확인
    - 기존 회의록 참고
    - ERD 및 업무프로세스 정의서를 바탕으로 업무 분석
    - 화면 정의서 재작성을 통한 현업 응대 회의 준비
    - 이번주 금요일 메일 발송이 목표
...
## 2021-11-12 업무일지
  - 워드 클라우드 팝업 
    - 부모 페이지에서 정상 작동하는 그래프가 팝업에서 그려지지 않았던 문제
    - popup.html에서 js 소스를 추가하면 바로 해결되는 문제였음
    - 화면단 소스를 수정해서 해결하려했던 1일 간의 노력이 허무했던 경험
    - 공통팀의 개발전 교육이 필요한 사례로 정리
    
  - 전체 화면에서 개발 필요 부분 파악
    - 현재 한개의 목록 페이지를 제외한 모든 목록 페이지 개발 완료
    - 팝업 중에서는 기술개발 쪽에서 구현이 되면 그 팝업을 사용하도록 세팅하면 모두 적용 완료 가능
    - 상세화면은 read 기능은 모두 구현 완료
    - 상세화면의 권한별 버튼 적용
    - 버튼별 업무 프로세스 적용
    - 상태값 반영 로직 적용
    - 기안 기능 관련 서비스 구현 필요
    - 기안 기능 관련 서식 확인 및 적용 필요
    - 출력 관련 보고서 양식 최종 확인 및 데이터 확인 후 최종 양식과 쿼리 작성 후 적용 필요
    - 결재 문서 보기 기능 구현 필요
    - 위원회로 승인 처리하는 프로세스 적용 필요
    - CUD 스크립트 구현 필요
    
  - 11월 15일 이후 일정
    - pi 확인 필요 사항 정리
      - 정리 전에 9월 7일 요청사항 중 가능한 부분 모두 반영
    - pi 확인 후 상세페이지 구현 시작
    - 구현에 필요한 DB 분석
    - 화면 분석, 설계에 대한 학습 및 소통 대비
    
    
---
## 2021-11-15 업무일지
  - 보유기술 선택 팝업
    - 다른 파트의 요청으로 신규 페이지 작성
    - 팝업의 경우 개발에 총 1시간 정도 소요
    - 만들어진 페이지를 재활용하는 경우 개발 속도가 상당히 빨라짐
    
  - 기안 기능 관련 소스 분석
    - 천과장님 소스에 의존하다 다른 개발자들의 소스를 보면서 소스 독해력 향상을 도모
    - 스프링 프레임워크에 대한 이해 및 자바 언어에 대한 이해가 부족함을 인지
    - 여가 시간을 통해 스프링, 자바 등에 대한 복습 및 심화 학습이 필요

    
---
## 2021-11-16 업무일지
  - 9월 7일 현업 의견 모두 반영
    - 그래프 클릭 이벤트에서 새창 포커스 문제 해결 필요
    - 목록 조회 조건을 모두 반영
  
  - PI 소통을 위한 메일 구성
    - 내일 오전에 설계자에게 먼저 문의 후 확인 절차를 거친 후 발송 예정
    - 설계자의 오만함을 통해 올바른 설계자 및 관리자의 자세를 배움
    - 프로젝트 리더로서 함양해야할 자세와 능력에 대한 고찰
    
    
---
## 2021-11-17 업무일지
  - 9월 7일 현업 요청 사항 반영 최종 확인
    - 일부 화면에 대한 내용을 제외한 적용된 부분에 대한 점검 완료
    - 그래프 클릭 이벤트에서 새창 포커스가 되도록 스크립트 추가(공통팀 협조로 해결)
  
  - PI 소통 메일 송부 실패
    - 설계자의 낮은 의사소통 능력으로 말미암아 불필요한 시간 할애 및 정신적, 육체적 에너지 과다 소모
    - 확인 요청 사항중 설계자 선에서 확인했던 사항은 하나도 없었음
    - PI와 직접 소통을 하려하자 바로 자기가 메일을 작성하겠다하고 이를 도와주기를 바람
    - 메일 송부 후 PI의 답변이 오기까지는 4시간도 채 걸리지 않음
    - 이런 소통을 미루는 이유가 이해가 안됨
    
  - 연장 후 프로젝트
    - 연장 계약서에 싸인
    - 연장후 정치로 개발현장에 남는 인력으로부터 스스로 보호할 능력이 필요
    - 기안, 결재문서보기, 문서출력, 상세화면 업무 프로세스별 CRUD 처리, 위원회 승인 요청
    - 최대한 일정 빨리 치기
...
## 2021-11-18 업무일지
  - 상세화면 텝별 로직 구현 준비
    - 업무 프로세스를 ERD 참조하여 분석
    - 진석필 부장님의 도움으로 기술개발 분야와 연계되는 부분을 이해함
    - 상태값의 중요성이 가장 큼
    - 권한으로 분리할 수 있는 부분은 메뉴에 대한 접근까지로 1차적인 분기를 처리
    - 화면단에서는 상태값에 따른 insert, update를 구분해서 수행해야함
    - 제한적으로 delete를 수행하는 쿼리를 적용
    - 이메일 자동 발신 기능 추가
    - 기안 기능 추가(소스분석 1회 완료)
    - 결재문서 보기 기능 추가(소스분석 1회 완료)
    - 문서출력 기능 추가(소스 분석 및 테스트 실시 1회 완료)
    
  - 실용화상세 담당자 추가
    - 사내/사외 구분 팝업 추가로 사외일 경우 수기 입력 가능하게 변경 필요
...
## 2021-11-19 업무일지
  - 상세페이지 개발
    ...
    - 사내 / 사외 구분 로직 확인
      - 구별 구현 방법에 대한 고민
      - AS-IS 와 같이 구현하기 위해서는 alert화면을 사용해야하기에 다른 방향으로 구상함
      - 셀렉트 박스에 사내 / 사외를 구분할 수 있는 상태로 설정
      - 쿼리에서는 사번이 00000000인 경우를 사외로 불러들임
      - 사외 담당자 입력시에는 사번이 00000000로 고정된 값을 입력
    ...
    - 전반적인 개발 순서
      ...
      - 권한별 로직 구현
      - 업무프로세스별 로직 구현
      - 기안, 결재문서보기, 문서출력, 이메일 전송 등의 부가 기능 구현
...
## 2021-11-22 업무일지
  - 상세페이지 개발
    - rowStatus 관련 소스 확인
    - rowStatus 적용하여 insert, update 분기 처리로 기존 오류 수정
    - 버튼 권한, 화면 권한 부여
    - 상태값적용을 가정한 CUD 구현 예정 및 기능 추가 테스트 예정
    
  - 다수의 개발자들의 연장 불가
    - 현재 프로젝트의 상태로 말미암아 대다수의 개발자가 이탈
    - 남은 페이지 개발을 위한 기술 습득이 필요
    
   
---
## 2021-11-23 업무일지
  - 상세페이지 개발 
    - 배부장님의 소스를 참고하여 스트립트 단의 소스 레이아웃 편집
    - CUD를 위한 테스트 코드 작성
    
  - 위원회 관련 소스 분석
  - 기술개발 파트의 개발 소스 분석 및 계약 만료전 문의 사항 정리
  - 연구노트 관련 공유
    - 진부장님의 연구노트 개발 관련 소스 공유
    - 개발에 필요한 자바 학습
      - 스레드, 스레드풀, 보스스레드, 워커스레드, 오브젝트
      - 일정에 따라 잔여 개발 부분을 맡을지 고민중
  
---
## 2021-11-24 업무일지
  - 상세페이지 개발
    - 위원회 승인 프로세스 이해 및 코드 적용
    - 승인 버튼 클릭시 위원회요청 테이블에 insert 처리하면 끝
    - 위원회 승인 절차 이후 프로세스는 최종 결정 단계에서 처리할 쿼리를 추후 전달하여 함께 처리하도록 하면 완료
    - 프로세스별로 분기 처리 및 유사한 프로세스지만 처리 로직이 상이한 경우 오류를 줄이는 것이 관건
    
  - 업무 프로세스 
    - 위원회 승인 프로세스와 마찬가지로 연관된 업무의 개발자와 소통이 중요
    - 사용할 테이블, 공통코드, 업무 로직 등을 공유하면 개발이 훨씬 수월함
...
## 2021-11-25, 26 업무일지
  - 상세페이지 개발
    - 보유기술 저장, 수정 기능 완료
    - 저장, 수정을 분기하는 로직을 화면단이 아닌 쿼리로 완성
    - merge 쿼리로 분기하는 것이 보다 효율적임
    - 업무적으로 등록요청, 등록반려, 승인요청, 작성중인 경우 프로세스 완성
    - 화면단 접근은 권한으로 통제하기에 화면단에서 제어할 필요는 없는 보유기술
    
  - 업무 PC 조립
    - 직접 구매한 부품들로 생애 첫 조립 완성
    - 생각보다 배선에서 시간을 많이 잡아먹었으나 연결에 오류없이 한번에 성공
    - 모니터 연결을 먼저 하지 않안 긴게 한번, 짧게 세번 경고음이 발생한 것을 깨닫게 됨
    - 최초 연결시 그래픽카드가 모니터를 인식하지 못했을 경우 발생하는 그래픽카드 오류임
    - 모니터 연결 성공 후 프로그램 설치 가능한 상태가 됨
    - 윈도우 설치 및 바이오스로 램 오버클럭 성공
    - 윈도우, MS 오피스 모두 인증키 인증 성공
    - 마우스 사용을 위한 로지텍 프로그램 설치 및 블루투스 USB 결합
  
  - 컴퓨터 교체 작업
    - ecl 제공 노트북과 모니터 회수
    - 개발에 사용하는 장비는 모든 개인 장비로 교체 완료함


---
## 2021-11-27 업무일지
  - IP 허가 시점까지 업무
    - IP가 재인가되기 전까지 개발업무는 불가
    - 업무 프로세스 확인, ERD 출력본 활용한 분석 등은 가능하여 해당 부분에 대한 확인 및 구조 파악 작업 예정
    - 계약 만료 개발자들의 소스 활용을 위한 조언, 팁들 전수 받는게 우선
    
  - 시즌2
    - 계약 연장으로 맞이할 시즌2를 함께 할 개발자의 최종명단 확인
    - 같은 팀으로 지낼 새로운 지식재산파트 담당 개발자와 친분 쌓기
    - 12월을 실제 개발 업무의 모든 부분은 마무리 할 수 있는 일정으로 진행
    - 현재 보유기술 쪽은 50%정도 상세 개발 진행중
    - 기술실용화는 상세페이지에서 탭별 분기별 로직 처리가 상당히 번거롭기 때문에 적어도 1주일 이상의 공수 필요
...
## 2021-11-29 업무일지  
  - ERD 확인
    - 보유기술, 기술실용화 ERD 확인
    - 필요없는 매핑 테이블 확인
    - 상태코드 값 추가 필요성 확인 중에 두 파트에 대한 문의 및 개발을 개발자 주도로 하기를 요청 받음
    - 추후 프로세스에 대한 보증이 필요한 시점
    - 상태값 관련 코드 파악 필요
    - 6개 컬럼 추가 관련 문의 필요
    - ERD상의 컬럼 들의 활용처 확인 필요
  
  - 파일서버 접속
    - 금요일 PC교체 후 아직 파일서버에 접근 권한이 생기지 않아 ERD 분석과 같은 업무 파악 수행
    - 업그레이드된 성능을 체감해보고 싶으나 프로그램 설치를 하지 못해 하지 못하고 있는 상황
    
  - 계약 만료 개발자들
    - 내일 떠나는 개발자들을 바라보는 마음
    - 함께 끝까지 하지 못하는 아쉬움과 이후 일정에 대한 막막함이 공존
    - 언제 다시 볼지 모르는 인연으로 내일까지 잘 마무리가 목표

---
## 2021-11-30 업무일지  
  - ERD 확인 및 테이블 구조 파악
    - 실제 업무 프로세스에 대한 명확한 분석이 필요
  
  - 시즌2
    - 계약 만료 개발자들 어시스트
    - 교체한 PC 세팅 작업
    - 오전에 파일서버 접속 가능해짐
    - 오후에 세팅 계속하여 TO-BE, AS-IS 구동 완료
    - 기타 다른 프로그램들 설치 및 정상 구동을 위한 세팅 필요
    - 자리 변경 가능 여부 문의 필요
...
## 2021-12-01 업무일지 
  ...
  - 보유기술, 기술성과분석 자기주도적 개발
    - PI와 직접 소통하면서 ERD 및 설계 파트의 부족한 점 수정, 보완하면서 개발 진행 필요
    - 최종 결과물에 대한 품질 검증을 위해 업무 프로세스에 대한 명확한 이해가 필요
    - 상태 코드와 버튼 활성화, 메뉴별 노출 항목 등에 대한 세부적인 확인 예정
    - 12월 내로 모든 페이지 개발 완료 목표
    
  - PI 소통 대비
    - ERD 및 화면단에서 사용하지 않는 컬럼들에 대한 조사 필요
    - 권한, 상태값, 업무 상황에 따른 화면 진행 상태에 대한 분석을 선행
...
## 2021-12-02 업무일지 
  - 업무파악
    - PI 문의 내용에 대한 답변
      - 실제 회의 내용과 차이가 있는 답변
      - 화면을 실제로 사용하는 현업과 의사소통이 필요함을 인지함
      - 미팅 전 파악해야할 요소들에 대한 정리가 필요
      - 다음주는 상세페이지 클리어를 위한 최종적인 업무 파악이 목표
      - 이와 더불어 ERD 및 공통코드 수정도 예정
    
    - 업무파악 전 화면
      - 도표 2개가 들어가는 목록 화면 개발
    
    - 업무 파악 이후 개발 일정
      - 상세페이지에 프로세스 적용 및 권한, 상태값에 따른 처리
      - 현업이 만족하고 오류없는 화면 구현이 목표
    
    
---
## 2021-12-03 업무일지 
  - PI와 소통 
    - 답변에 대한 추가 질의 및 현업 미팅 제의
    - 실용화통계현황 목록 페이지의 차트 레이아웃 완성 및 쿼리 완성
    - 월요일에 쿼리를 차트에 적용 예정
    - 화면 사용에 대한 최종 확인 후 세부 내용 수정 후 커밋 예정
    
  - 업무 상세 파악
    - 보유기술 등록에서 1:n 관계 확인 필요
    - 실용화관리 프로세스 상세확인 필요
...
## 2021-12-04 업무일지 
  - 다음주 현업 회의 대비
    - 보유기술, 기술실용화 관련 사전 지식 공부
    - ERD 관련 내용 학습 필요
    - DB 관련 내용 학습 필요
    - 현업, PI와 소통에 있어 부족하지 않은 모습을 준비해야함

---
## 2021-12-05 업무일지 
  - 12월 2주차 계획
    - 현업 회의 준비 및 업무 프로세스 최종 확인 및 정리
    - DB 수정 및 상태값 수정 등 프로세스 구현을 위한 작업 마무리
    - 마지막 목록 페이지 차트 완료해서 목록 페이지 클리어
    
  - DB 관련 업그레이드
    - ERD 분석 능력
    - DA# 활용 ERD 수정 및 테이블 작성 쿼리 생성 후 적용
    - DB 관련 프로그램 활용법 연습
...
## 2021-12-06 업무일지 
  ...
  - DA#5
    - DB 설계 프로그램으로 현재 프로젝트에서 사용중이 툴
    - ERD, 논리 테이블, 물리 테이블 작성 가능
    - 용어 관리 기능
    - 레이아웃 설정 가능
    - 제공되는 기본 기능에 대한 학습으로 필요시 사용 예정
    
  - 코딩
    - 화면과 백앤드 구현 등의 코딩을 할 때는 재미가 있으나 화면 분석, 설계시에는 답답함이 아직 많음
    - 업무 분석과 그에 맞는 설계에 대한 명확한 인식이 없기에 비롯된 상황으로 학습과 연습이 필요
    
    
---
## 2021-12-07 업무일지 
  - 업무파악 및 정리 완료
    - 보유기술, 기술실용화 업무 파악 완료 및 코드 분석 정리 완료
    - 세부적인 질문 사항에 대한 정리는 내일 실시 예정
    - ERD 수정을 위한 DB 관련 학습 필요
    
  - 차트 2개 포함된 목록 페이지 개발 완료
    - 실용화현황 목록 페이지 개발 완료
    - 차트 2개는 현업에서 요청한 새로운 요구사항을 반영한 것으로 현업 회의 후 변동될 가능성이 있음
    - 차트 커스터마이징이 전보다 다양하고 빠르게 이루어짐
    
  - DA#5 활용 
    - ERD 수정 및 테이블 구조 변동에 있어 필수적인 프로그램
    - 현재 보유기술쪽 매핑 테이블 삭제, 담당자 이력 테이블 삭제
    - 기술실용화 쪽 업무처리를 위한 컬럼 추가, 변경전이 붙은 컬럼명 모두 수정
    - 현재의 테이블 구조를 업무프로세스와 동기화 작업이 필요
    
---
## 2021-12-08 업무일지   
  - PI 소통 내용 정리
    - 업무 분석 내용중 협의 필요 사항 정리
    - 프로세스에 대한 명확한 이해를 위함
    - ERD 상의 수정 사항을 직접 반영하기 위한 공부가 필요
    
  - DA#5
    - DB 개발을 직접할 수 있는 프로그램 활용 및 개념 학습
    - 보유기술, 기술실용화를 시작으로 분석하고 화면 설계 내용을 데이터베이스에 적용
    - 단어 표준화, 용어 표준화, 표준 사전 활용 방법 등 전반적인 프로세스 숙달 필요
    - AR 연결 가능하도록 작업중
    
  - 차트 렌더링 오류 수정
    - 차트 아이디 중복으로 발생한 오류로 아이디를 다르게 순번을 줘서 오류 수정 완료


---
## 2021-12-09 업무일지      
  - 업무 확정
    - 보유기술, 기술실용화 파트 화면 분석 및 설계, 개발까지 전담하는 것으로 연이사님과 송부장님 함께 회의를 제안
    - 연이사님의 일정으로 회의하지 못했지만 다음 주에 협의 필요
    
  - 대,중,소분류 적용
    - 쿼리 수정 후 반영이 안된 페이가 있어 수정
    
  - 시즌2 대비
    - 윤광진 차장님과 첫 대화
    - 기술통합 쪽 이야기 및 보유기술, 기술실용화 파트 단독 진행에 대한 조언을 구함
    - 앞으로 소통을 이어가며 도움을 받으면 성장에 큰 힘이 될 것으로 기대됨
...
## 2021-12-11 업무일지      
  - 설계 준비
    - 2022년도 SQLP 시험 대비 
      - 시험 계획 수립
      - 주단위 학습 일정 마련
      - 보유기술, 기술실용화 관련 설계 수정을 위한 필요 내용 선수 학습
      - 기술개발 파트 분석을 위한 준비 과정
      
  - 12월 내 일정 파악
    - 24일과 31일은 쉬는 금요일로 3, 4째주는 실제 근무일수가 4일씩임
    - 남은 근무일수는 13일로 해결 가능한 범위의 업무 파악 및 개발 수행
    - 현업과 업무 협의 이후 보유기술 쪽 모든 기능 구현을 우선으로 작업 시작 예정
    - 오즈 뷰어를 활용한 단건, 다건 문서 출력 기능과 위원회로 넘기는 프로세스 구현
    - 업무 프로세스별 버튼 및 상태에 따른 상세 구현 필요
    - 보유기술 완료 후 기술실용화 파트 상세 구현 시작
    - 기술 실용화 파트는 진행상태가 더욱 세분화 되어있기에 상세페이지 처리에 있어 복잡
    - 보유기술 및 기술실용화 설계 테이블들에 수정이 필요한 경우 직접 ERD 수정 및 테이블 수정이 필요
    
---
## 2021-12-12 업무일지
  - SQLP 시험 준비 시작
    - 주말을 마치고 난 저녁 10시 40분에 시작
    - SQL 전문가 가이드를 첫 교재로 시작해서 정독하면 정리하는 것을 시작 과정으로 함
    - 개발 진행에 있어 필요한 부분이 생기면 해당 파트에 집중
    - 3월 초 시험이기에 준비 기간은 3달이 채 되지 않음
    - 교재의 분량이 매우 많기에 현업에 개발 일정을 치면서 학습을 병행하기는 수월하지는 않겠지만 시도!
    
    
---
## 2021-12-13 업무일지    
  - PI 소통
    - 확인 사항에 대한 확인 및 대중소분류 조회 정상화
    - 현업 미팅 재요청
    
  - 배포 관련 이슈  
    - 스프링 의존성 주입 관련 내용 학습 필요
    - 프로젝트의 전체적인 구조에 대한 이해
    - 리눅스 명령어 중 자주 활용되는 사례를 노충호 대리님으로부터 배움
    
  - 관리자 부재
    - 파트 전반적인 관리를 담당하는 사람이 없이 개발을 자기주도적으로 진행해야함
    - 업무 파악, DB 재설계부터 세부 페이지 개발까지 모두 스스로 해결해야함
    
---
## 2021-12-14 업무일지     
  - 업무 범위 확정
    - 보유기술, 기술실용화 파트 분석, 설계부터 완료 문서까지 전체적으로 전담
    - 테이블 수정 등에는 천이사님과 협업이 필요
    - 현업 미팅 후 확정된 업무 프로세스를 기준으로 개발에 속도를 낼 예정
    - 필요한 DB 수정 작업 선행 필요
    
  - 기술통합 전체 프로세스 확인
    - 담당 파트 설계에 대한 책임을 갖게 된 만큼 업무 흐름에 맞는 개발을 위함
    - 기술통합 메인 테이블에 대한 의문과 설계에 대한 궁금증도 존재
    - 전체적인 프로세스를 추가적으로 확인
    - DB 수정을 위한 작업 내용도 추가적으로 확인 후 하나씩 적용 예정
...
## 2021-12-15 업무일지     
  - 현업 미팅 확정
    - 내일 오후 1시로 최종 결정
    - 일정에 변동이 생기고 회의실 변경으로 총 3번 PMS에 회의실 신청
    - 회의 시간도 1시간 밖에 없어서 회의자료를 PPT로 만듬
    - 최종적으로 점검한 이후에 내일 오전에 회의 담당자들에게 메일로 전송 예정
    - 현업은 회의자료를 출력물로 받을 수 있게 세팅 예정
    
  - 프로세스 정의서
    - 기술통합 전체 프로세스 이해를 위한 작업
    - ERD와 실제 테이블의 차이가 존재 함을 인지
    - 올바로 구현되기 위한 상태를 머리속으로 그려봄
    - 다른 파트의 개발자들의 조언을 통해 이상적인 ERD 작성 연습 예정
    
---
## 2021-12-16 업무일지     
  - 현업 회의 완료
    - 회의 안건, 준비, 소통 모두 스스로 해결한 첫 사례
    - 설계 단의 문제 제기는 모두 예상했던바와 일치하여 수정
    - 신규 요청 사항 확인으로 인한 변경 사항도 접수
    - 노터빌리티 어플 활용한 회의 내용 녹음
    - 회의록 정리 및 설계 개선 방안 정리 후 실무자 공유 예정
    
  - 설계단부터 개발까지
    - 책임 권한도 위임 받는 것이 장기적으로 보다 효율적이고 도움이 되는 길로 판단이 됨
    - 조만간 송부장님으로부터 완벽한 독립을 공식적으로 이루어야할 필요가 있음
    
  
---
## 2021-12-17 업무일지  
  - 회의 내용 정리
    - 노터빌리티 녹음 내용을 근거로 세부사항까지 모두 정리
    - 기억에 의한 것보다 정확히 회의 내용을 정리할 수 있어 추후 활용성이 입증됨
    - 회의 내용을 정리하고 화면단에서 체크하면서 이제까지 확인없이 진행해왔던 부분에 대한 검증도 다시금 필요함을 인지
    
  - 설계 시기에 현업과 소통 필요성
    - 업무에 대한 파악과 그에 부합하는 화면 구성
    - 화면 구성을 하면서 업무 프로세스에 맞는 데이터 베이스 구축을 위한 확인 과정이 필요
    - 화면에 표시되는 모든 내용에 있어 의문이 드는 점이 없도록 명확하고 세부적인 확인 과정이 필요함
    - 현재는 그런 과정없이 개발이 되었기에 다시 원점으로 가야하는 부분들이 다수 존재
    - 이런 경험을 바탕으로 향후 개발에 있어 웹개발의 각 시기에 필수적인 절차를 지켜나갈 것을 각인시킴
  
  - 설계와 관련된 실무 능력
    - DA#, ERD, Oracle, 용어신청, 표준화, DB 등록 및 수정 등 설계단에서 필요한 작업과 능력에 대한 보충이 필요
    - 웹 개발의 전체적인 경험을 쌓는 소중한 기회로 여기고 개발 진행 예정
    - 기존 설계자 배제 및 단독 파트로 인정받지 않으면 계약 파기 고려

---
## 2021-12-20 업무일지  
  - DB 데이터 복구
    - STP DB에 수정 요청한 사항이 반영되지 않음을 확인
    - DA# 활용을 위한 권한 확인 및 재신청 작업
    - 프로세스 정의서, 화면, ERD, DB 테이블 등의 일관성이 없는 부분 확인
    - 보유기술, 기술실용화 파트의 단독 진행으로 위와 같은 불일치 상황 해소 가능
  
  - 설계단 작업 확인
    - 표준 단어 신청, 용어 신청, 도메인 관리, 테이블 수정 등의 작업을 위한 워크플로우 확인
    - STP는 추후 연동 예정으로 일단 개발 DB의 설계를 직접 수행하고 변동사항 반영한 개발 진행
    
  - 연말 목표
    - 테이블 수정사항 반영 및 변동된 구조로 모두 디비 변경 적용
    - 분석 및 설계는 2021년 이내에 마무리 목표
    - 1월부터는 개발만 집중할 수 있도록 세팅
...
>>>>>>> parent of 31a945b (Update Brad's Web Devlelopment Diary.md)
